...
class AQA_RawFramesRecord(object):
...
class AQA_SingleClassDataset(Dataset):
    def __init__(self,
                 action_dir,
                 subset,
                 label_name,
        ...
        # number of consecutive frames, old_length: number of frames per segment before sampling
    ...
    def load_annotations(self):
        ...
        def _get_frame_num(action_dir, vid):
            rgb_dir = osp.join(action_dir, 'frames', vid)
            flow_dir = osp.join(action_dir, 'optical_flow', vid)
        ...
        list_path = osp.join(self.action_dir, 'split', '{}.txt'.format(self.subset))
        sample_list = load_name_list(list_path)
        label_path = osp.join(self.action_dir, 'labels', '{}.json'.format(self.label_name))
        with open(label_path, 'r') as file:
            labels = json.load(file)
        video_infos = []
        for vid in sample_list:
            num_frames = _get_frame_num(self.action_dir, vid)
            video_infos.append(AQA_RawFramesRecord([vid, num_frames, labels[vid]]))
    ...
    def _get_frames(self, record, image_tmpl, modality, indices, skip_offsets):
        images = list()
        floder_dict = {'RGB': 'frames', 'Flow': 'optical_flow'}
            ...
            for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                if p + skip_offsets[i] <= record.num_frames:
                    seg_imgs = self._load_image(osp.join(
                        self.action_dir, floder_dict[modality], record.name),
                        image_tmpl, modality, p + skip_offsets[i])
                else:
                    seg_imgs = self._load_image(
                        osp.join(self.action_dir, floder_dict[modality], record.name),
    ...
    def __getitem__(self, idx):
        ...
        data = dict(num_modalities=DC(to_tensor(len(self.modalities))),
                    gt_label=DC(to_tensor(record.label).float(), stack=True,
                                pad_dims=None), sample_name=DC(record.name, cpu_only=True))
...
def load_name_list(path, pair_list=False):
